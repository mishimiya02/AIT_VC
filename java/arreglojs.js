...
function guardarValuacionEnHistorial(nombre, numeroContrato, totalAcumulado, primerMesImporte, meses) {
    const fecha = new Date().toLocaleString();

    if (historialValuaciones.length >= 3) {
        historialValuaciones.shift(); // Eliminar la más antigua si ya hay 3
    }
    ...
    historialValuaciones.push({
        ...
        primerMesImporte: parseFloat(primerMesImporte).toFixed(2),
        fecha,
        meses // <-- Aquí se guarda el array completo con todos los datos mes a mes
    });
}
...
if (i === 0) {
    ...
    mesActual.setMonth(mesActual.getMonth() + 1);
                ...
                guardarValuacionEnHistorial(nombre, numeroContrato, valorTotalContrato, importeBruto, data);
